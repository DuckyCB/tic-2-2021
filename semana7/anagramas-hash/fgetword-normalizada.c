...
static char normalizar(wchar_t wc)  // devuelve la vocal no acentuada, o convierte eñe en ene (min y may)
{
...
static wchar_t * wcfgetword (FILE *fp, wchar_t *word, int maxword)	// carga una palabra nueva en word, retorna word o NULL
{
...
fgetword_lower_alpha_normal(FILE *fp, char *word, int maxword)
{
    ...
    for (;;) {  // loop que es cortado por un return por fin de archivo o de palabras válidas
        ...
        *s = '\0';
        ...
        // si no pasó por un return, sigo iterando hasta leer una palabra "válida"
    }
}
